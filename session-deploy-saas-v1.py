...
login_session_url = "https://" + login_session_tenant + "/api/v1/auth/login"
...
login_session_post = login_session.post(login_session_url, headers=login_session_headers, data=login_session_payload)
token = (login_session_post.headers['Authorization'])
print('login success')

# SESSION CONFIGURATION API INFO
session_config_url = "https://" + login_session_tenant + "/api/orchestrate/v3/agents/session"
api_headers = {'Content-Type': 'application/vnd.api+json', 'Accept': 'application/json', 'Authorization': token}

# AGENT NAME
print(agentName)
# GET AGENT ID FROM API LOOKUP FUNCTION

agent_list_url = "https://" + login_session_tenant + "/api/orchestrate/v3/agents"
...
def add_session():
    ...
    for items in session_json:
        ...
        meta_url = "https://" + login_session_tenant + "/api/v2/bulk/insert/monitored-objects/meta"
        ...
        # POST SESSION DATA TO API AS JSON
        metadata_session_post = metadata_session.post(meta_url, headers=api_headers, data=json.dumps(session_data))
        # PRINT RESPONSE
